...
layout = dbc.Modal([
            ...
            dbc.ModalFooter([
                dbc.Button("Cancelar", id="cancel_button_novo_adv", color="danger"),
                dbc.Button("Salvar", id="save_button_novo_adv", color="success")
            ])
        ], id="modal_novo_adv", size="lg", is_open=False)
...
@app.callback(
    ...
    Input('save_button_novo_adv', 'n_clicks'),
...
def novo_adv(n, dataset, nome, oab, cpf):
    ...
    if n:
        ...
        df_adv.loc[df_adv.shape[0]] = [nome, oab, cpf] ## Usar o .shape permite inserir o registro como a última linha do dataframe
